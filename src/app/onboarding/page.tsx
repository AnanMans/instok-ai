...
// ── Preview image sets per archetype ──────────────────────────────────────────
const ARCHETYPE_PREVIEW_IMGS: Record<string, string[]> = {
...
export default function Page() {
  ...
  const _firstCatMatch = categories.find(cat => CATEGORY_IMAGES[cat])
  const _previewImgs = _firstCatMatch ? CATEGORY_IMAGES[_firstCatMatch] : (ARCHETYPE_PREVIEW_IMGS[effectiveArchetype] ?? ARCHETYPE_PREVIEW_IMGS.minimal)
  const previewStore = { id: 'preview', name: displayBrand.storeName || brandName || '—', slogan: displayBrand.slogan || '', colors: customColors, archetype: effectiveArchetype, whatsapp_number: '', lang: (lang ?? 'ar') as 'ar' | 'he' }
  const previewProducts = [
    { id: 'p1', name: t.s4Prod1, price: 149, image_url: uploadedImage || _previewImgs[0] || '' },
    { id: 'p2', name: t.s4Prod2, price: 199, image_url: _previewImgs[1] || '' },
  ]
          ...
          <div key={step} className="step-enter" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#080808', position: 'relative', overflowX: 'clip', width: '100%', maxWidth: '100%', WebkitOverflowScrolling: 'touch' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '44px 0 6px', overflow: 'hidden', minHeight: 0, minWidth: 0, maxWidth: '100%' }}>
              ...
              <div style={{ width: '240px', height: '420px', maxWidth: '100%', minWidth: 0, overflowX: 'hidden', borderRadius: '24px', overflow: 'hidden', margin: '0 auto', flexShrink: 0, border: '2px solid rgba(255,255,255,0.15)', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}>
                <div style={{ width: '375px', transform: 'scale(0.64)', transformOrigin: 'top left', pointerEvents: 'none' }}>
