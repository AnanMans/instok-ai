...
function getConfig(archetype: string, c0: string, c1: string, c2: string): ArchCfg {
  ...
  switch (archetype) {
    ...
    case 'beauty':
      return {
        pageBg: lightBg, heroBg: `linear-gradient(160deg, ${c0}20, ${c1}15)`,
        ...
        cardBg: `${lightBg}cc`, cardBorder: `${c0}20`, cardRadius: '12px',
        ...
        sectionTitleColor: `${c0}80`, pillBg: `${c0}18`, pillBorder: `${c0}30`, pillColor: c0,
      }
    case 'streetwear':
      return {
  ...
  return (
    <div style={{ background: heroBg, paddingTop: pm ? '0' : '56px' }}>
      <div style={{ padding: '48px 24px 40px', textAlign: 'center' }}>
        ...
        {store.slogan && <p style={{ fontSize: '13px', color: `${c0}aa`, marginBottom: '28px', lineHeight: 1.6 }}>{store.slogan}</p>}
          ...
          <div style={{ background: cfg.sectionBg, border: `1px solid ${cfg.sectionBorder}`, borderRadius: '16px', padding: '16px 18px' }}>
            ...
            {store.delivery_type && (
              <div style={{ marginBottom: paymentMethods.length > 0 ? '12px' : '0' }}>
                ...
                {store.delivery_areas && <p style={{ fontSize: '12px', color: cfg.mutedColor }}>{store.delivery_areas}</p>}
              </div>
            )}
