...
function getConfig(archetype: string, c0: string, c1: string, c2: string): ArchCfg {
  ...
  switch (archetype) {
    case 'luxury':
      return {
        pageBg: '#050505', heroBg: `linear-gradient(160deg, #0d0d0d, ${getDarkVariant(c0)})`,
        navBg: '#050505', navBorder: `${c0}18`, navTextColor: c0,
        textColor: c0, mutedColor: `${c0}60`,
        sectionBg: '#0a0a0a', sectionBorder: `${c0}15`,
        cardBg: `${c0}08`, cardBorder: `${c0}15`, cardRadius: '2px',
        ...
        sectionTitleColor: `${c0}70`, pillBg: `${c0}15`, pillBorder: `${c0}30`, pillColor: c0,
      }
    case 'gaming':
      return {
        pageBg: '#050510', heroBg: `linear-gradient(135deg, #0d0020, ${c2}, ${c0}40)`,
        navBg, navBorder: `${c0}60`, navTextColor: navText,
        textColor: '#fff', mutedColor: 'rgba(255,255,255,0.4)',
        sectionBg: navBg, sectionBorder: `${c0}30`,
        cardBg: `${c0}18`, cardBorder: `${c0}30`, cardRadius: '8px',
        ...
        sectionTitleColor: c0, pillBg: `${c0}20`, pillBorder: `${c0}50`, pillColor: c0,
      }
    case 'beauty': {
      ...
      return {
        pageBg: softBg, heroBg: `linear-gradient(160deg, ${softBg}, ${accent}18)`,
        navBg: bNavBg, navBorder: `${accent}30`, navTextColor: bNavTxt,
        textColor: '#111', mutedColor: 'rgba(0,0,0,0.45)',
        sectionBg: softBg, sectionBorder: `${accent}20`,
        cardBg: '#ffffff', cardBorder: `${accent}18`, cardRadius: '12px',
        cardExtra: { boxShadow: `0 4px 20px ${accent}12` },
        nameStyle: { fontWeight: 600, fontSize: '12px', color: '#444' },
        priceStyle: { fontWeight: 700, color: accent, fontSize: '14px' },
        sectionTitleColor: `${accent}70`, pillBg: `${accent}18`, pillBorder: `${accent}30`, pillColor: accent,
      }
    }
    case 'streetwear':
      return {
        pageBg: '#0a0a0a', heroBg: c2,
        navBg, navBorder: 'transparent', navTextColor: navText,
        textColor: '#fff', mutedColor: 'rgba(255,255,255,0.35)',
        sectionBg: '#111', sectionBorder: 'rgba(255,255,255,0.06)',
        cardBg: '#111', cardBorder: 'transparent', cardRadius: '0px',
        ...
        sectionTitleColor: 'rgba(255,255,255,0.3)', pillBg: 'rgba(255,255,255,0.05)', pillBorder: 'rgba(255,255,255,0.1)', pillColor: '#fff',
      }
    case 'minimal':
      return {
        pageBg: '#fff', heroBg: `${c0}08`,
        navBg, navBorder: `${c0}20`, navTextColor: navText,
        textColor: '#111', mutedColor: '#999',
        sectionBg: '#fafafa', sectionBorder: '#efefef',
        cardBg: '#fff', cardBorder: 'transparent', cardRadius: '4px',
        nameStyle: { fontWeight: 600, fontSize: '11px', color: '#777' },
        priceStyle: { fontWeight: 600, color: c0, fontSize: '13px' },
        sectionTitleColor: '#bbb', pillBg: '#f5f5f5', pillBorder: '#e8e8e8', pillColor: '#555',
      }
    case 'restaurant':
      return {
        pageBg: navBg, heroBg: `linear-gradient(135deg, ${getDarkVariant(c0)}, ${c0}55)`,
        navBg, navBorder: `${c0}30`, navTextColor: navText,
        textColor: navText, mutedColor: navText === '#ffffff' ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.4)',
        sectionBg: navBg, sectionBorder: `${c0}18`,
        cardBg: `${c0}12`, cardBorder: `${c0}18`, cardRadius: '12px',
        nameStyle: { fontWeight: 600, fontSize: '13px', color: navText },
        priceStyle: { fontWeight: 700, color: c0, fontSize: '18px' },
        sectionTitleColor: `${c0}80`, pillBg: `${c0}18`, pillBorder: `${c0}35`, pillColor: c0,
      }
    case 'tech':
      return {
        pageBg: navBg, heroBg: `linear-gradient(160deg, ${getDarkVariant(c0)}, ${getDarkVariant(c1)})`,
        navBg, navBorder: `${c0}20`, navTextColor: navText,
        textColor: navText, mutedColor: navText === '#ffffff' ? 'rgba(255,255,255,0.35)' : 'rgba(0,0,0,0.35)',
        sectionBg: navBg, sectionBorder: `${c0}20`,
        cardBg: `${c0}18`, cardBorder: `${c0}20`, cardRadius: '6px',
    ...
    default:
      return {
        pageBg: lightBg, heroBg: `linear-gradient(160deg, ${c0}18, ${c1}12)`,
        navBg, navBorder: `${c0}20`, navTextColor: navText,
        textColor: lightCardText, mutedColor: lightCardText === '#000000' ? 'rgba(0,0,0,0.45)' : 'rgba(255,255,255,0.45)',
        sectionBg: lightBg, sectionBorder: `${c0}20`,
        cardBg: `${lightBg}ee`, cardBorder: 'transparent', cardRadius: '4px',
        ...
        sectionTitleColor: `${c0}80`, pillBg: `${c0}20`, pillBorder: `${c0}40`, pillColor: c0,
      }
  }
}
...
type HeroBase = { store: StoreData; c0: string; heroBg: string; ar: boolean; pm: boolean }

function LuxuryHero({ store, c0, heroBg, ar, pm }: HeroBase) {
  return (
...
function GamingHero({ store, c0, heroBg, ar, pm }: HeroBase & { c1: string }) {
...
function BeautyHero({ store, c0, c1, heroBg, ar, pm }: HeroBase & { c1: string }) {
  const accent = isVeryLight(c0) ? c1 : c0
  const ctaTxt = getTextColor(accent)
  return (
    <div style={{ background: heroBg, paddingTop: pm ? '0' : '56px' }}>
      <div style={{ padding: '48px 24px 40px', textAlign: 'center' }}>
        ...
        {store.slogan && <p style={{ fontSize: '13px', color: `${accent}99`, marginBottom: '28px', lineHeight: 1.6 }}>{store.slogan}</p>}
...
function StreetwearHero({ store, c0, heroBg, ar, pm }: HeroBase) {
  ...
  return (
    <div style={{ background: heroBg, paddingTop: pm ? '0' : '56px', borderBottom: `3px solid ${c0}` }}>
      <div style={{ padding: '40px 20px 32px', textAlign: 'center' }}>
        ...
        {store.slogan && <p style={{ fontSize: '12px', color: heroTextColor, opacity: 0.5, textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '28px' }}>{store.slogan}</p>}
...
function MinimalHero({ store, c0, heroBg, ar, pm }: HeroBase) {
  return (
    <div style={{ background: heroBg, paddingTop: pm ? '0' : '56px', borderBottom: '1px solid #efefef' }}>
      <div style={{ padding: '56px 32px 48px', textAlign: 'center' }}>
        ...
        {store.slogan && <p style={{ fontSize: '13px', color: '#aaa', fontWeight: 300, marginBottom: '36px' }}>{store.slogan}</p>}
...
function RestaurantHero({ store, c0, heroBg, ar, pm }: HeroBase) {
  return (
    <div style={{ background: heroBg, paddingTop: pm ? '0' : '56px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '44px 24px 36px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <a href="#products" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', border: `1px solid ${c0}50`, color: c0, borderRadius: '20px', padding: '11px 24px', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>
            📍 {ar ? 'استلام' : 'איסוף'}
          </a>
        </div>
      </div>
...
function TechHero({ store, c0, heroBg, ar, pm }: HeroBase) {
  const ctaText = getTextColor(c0)
  return (
    <div style={{ background: heroBg, paddingTop: pm ? '0' : '56px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '44px 24px 36px', position: 'relative', zIndex: 1, textAlign: 'center' }}>
        ...
        {store.slogan && <p style={{ fontSize: '12px', color: `${c0}80`, fontFamily: 'monospace', marginBottom: '12px' }}>{store.slogan}</p>}
...
function CreatorHero({ store, c0, c1, heroBg, ar, pm }: HeroBase & { c1: string }) {
  const ctaText = getTextColor(c0)
...
const fmt = (n: number) => '₪' + n.toLocaleString('en-US')
  ...
  return (
    <div dir="rtl" className={fontClass} style={{ height: 'auto', background: cfg.pageBg, color: cfg.textColor, overflowX: 'hidden', paddingBottom: (!pm && waNumber) ? '100px' : '0' }}>
      <style>{`
      ...
      <nav dir="rtl" style={{ position: pm ? 'relative' : 'fixed', top: 0, left: 0, right: 0, zIndex: 50, background: cfg.navBg, backdropFilter: 'blur(12px)', borderBottom: `1px solid ${cfg.navBorder}`, padding: '0 16px', height: '56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        {archetype === 'luxury' ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div style={{ width: '34px', height: '34px', borderRadius: archetype === 'streetwear' || archetype === 'minimal' ? '4px' : '50%', background: `linear-gradient(135deg,${c0},${c1})`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', boxShadow: archetype === 'gaming' || archetype === 'tech' ? `0 0 10px ${c0}50` : 'none' }}>
              {store.logo_url
                ? <img src={store.logo_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
      ...
      {/* ── Hero ───────────────────────────────────────────────── */}
      {archetype === 'luxury'     && <LuxuryHero     store={store} c0={c0}           heroBg={cfg.heroBg} ar={ar} pm={pm} />}
      {archetype === 'gaming'     && <GamingHero     store={store} c0={c0} c1={c1}   heroBg={cfg.heroBg} ar={ar} pm={pm} />}
      {archetype === 'beauty'     && <BeautyHero     store={store} c0={c0} c1={c1}   heroBg={cfg.heroBg} ar={ar} pm={pm} />}
      {archetype === 'streetwear' && <StreetwearHero store={store} c0={c0}           heroBg={cfg.heroBg} ar={ar} pm={pm} />}
      {archetype === 'minimal'    && <MinimalHero    store={store} c0={c0}           heroBg={cfg.heroBg} ar={ar} pm={pm} />}
      {archetype === 'restaurant' && <RestaurantHero store={store} c0={c0}           heroBg={cfg.heroBg} ar={ar} pm={pm} />}
      {archetype === 'tech'       && <TechHero       store={store} c0={c0}           heroBg={cfg.heroBg} ar={ar} pm={pm} />}
      {archetype === 'creator'    && <CreatorHero    store={store} c0={c0} c1={c1}   heroBg={cfg.heroBg} ar={ar} pm={pm} />}
      ...
      <div id="products" style={{ padding: '20px 16px 0', maxWidth: '600px', margin: '0 auto' }}>
        ...
        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '48px 24px', background: cfg.sectionBg, borderRadius: '16px', border: `1px solid ${cfg.sectionBorder}`, marginBottom: '24px' }}>
            ...
            <p style={{ fontSize: '12px', color: cfg.mutedColor }}>{ar ? 'تابعونا للتحديثات' : 'עקבו אחרינו לעדכונים'}</p>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: archetype === 'streetwear' ? '2px' : '12px', marginBottom: '24px' }}>
            {products.map((p, i) => (
              <div key={p.id ?? i} className="prod-card"
                ...
                style={{ background: cfg.cardBg, borderRadius: cfg.cardRadius, overflow: 'hidden', cursor: pm ? 'default' : 'pointer', border: `1px solid ${cfg.cardBorder}`, ...cfg.cardExtra }}>
                {archetype === 'restaurant' ? (
                  ...
                  <>
                    <div style={{ width: '100%', aspectRatio: '1', overflow: 'hidden', background: isLight ? '#f5f5f5' : '#111' }}>
                      <img src={p.image_url || imgFallback} alt={p.name} onError={e => { (e.target as HTMLImageElement).src = imgFallback }}
                        style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                    </div>
      ...
      <div style={{ padding: '0 16px 40px', maxWidth: '600px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        {/* ── WhatsApp button (inline in preview, fixed in live) ─ */}
        {waNumber && pm && (
          <div style={{ background: '#22c55e', borderRadius: '14px', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
            <span style={{ fontSize: '18px' }}>💬</span>
            <span style={{ fontSize: '14px', fontWeight: 700, color: '#fff' }}>{ar ? 'تواصل عبر واتساب' : 'צור קשר בוואטסאפ'}</span>
          </div>
        )}
      ...
      {/* ── Sticky WhatsApp button (live only) ─────────────────── */}
      {waNumber && !pm && (
