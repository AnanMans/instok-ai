  ...
  switch (archetype) {
    ...
    case 'gaming':
      return {
        ...
        nameStyle: { fontWeight: 700, fontSize: '12px', color: navText, letterSpacing: '0.02em' },
    ...
    case 'minimal':
      return {
        ...
        cardBorder: 'transparent',
        cardRadius: '4px',
        nameStyle: { fontWeight: 500, fontSize: '11px', color: '#777', letterSpacing: '0.08em', textTransform: 'uppercase' as const },
    ...
    default:
      return {
        ...
        nameStyle: { fontWeight: 600, fontSize: '12px', color: lightCardText, letterSpacing: '0.01em' },
        priceStyle: { fontWeight: 700, color: lightPrice, fontSize: '14px' },
        sectionTitleColor: `${c0}80`,
        pillBg: `${c0}20`,
        pillBorder: `${c0}40`,
...
function StreetwearHero({ store, c0, heroBg, ar }: { store: StoreData; c0: string; heroBg: string; ar: boolean }) {
...
function CreatorHero({ store, c0, c1, heroBg, ar }: { store: StoreData; c0: string; c1: string; heroBg: string; ar: boolean }) {
  const ctaText = getTextColor(c0)
...
export default function StoreTemplate({
  ...
  return (
    <div dir="rtl" className={fontClass} style={{ minHeight: '100vh', background: cfg.pageBg, color: cfg.textColor, overflowX: 'hidden', paddingBottom: (!preview && waNumber) ? '80px' : '0' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: archetype === 'streetwear' ? '2px' : '12px', marginBottom: '24px' }}>
            {products.map(p => (
              <div key={p.id} className="prod-card"
                ...
                style={{ background: cfg.cardBg, borderRadius: cfg.cardRadius, overflow: 'hidden', cursor: preview ? 'default' : 'pointer', border: `1px solid ${cfg.cardBorder}`, ...cfg.cardExtra }}>
                {archetype === 'restaurant' ? (
      ...
      {/* ── Product drawer ──────────────────────────────────────── */}
      {!preview && drawerOpen && drawerProduct && (
